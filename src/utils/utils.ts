...
// 当前时间格式化
export function currentSelectTime(date:any, format = 'YYYY-MM-DD') {
  return moment(date).format(format)
}
function zeroHandle(num:number){
  if(num < 10){
    return '0' + num
  }else{
    return num.toString()
...
// 根据type获取不同的基本数据
export function timeDataArr(type = 'day', startTime:string, endTime:string) {
  let dataObj:timeProp = {
    xArr: [],
    baseArr: []
  }
  if(type === 'day'){
    ...
    for(let i = 1; i < 8; i++){
      let data = moment(startTime).subtract(Number(week)-i, 'days').format('YYYY-MM-DD')
      let obj = {
        name: data,
        value: 0
      }
    ...
    let currentDate:any  = moment(startTime).valueOf()
...
export const parseQuery=(query:string)=>{
  const paramsString = query.substring(1)
  const searchParams = new URLSearchParams(paramsString)
  return searchParams;
}
// 缓存设置有效期
export const timeLocalStorage = {
  setItem: function (key:string, value:any, dayNum:number) {
